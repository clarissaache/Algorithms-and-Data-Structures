...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  // (*newBoard).board = malloc( h * w * sizeof((*newBoard).board));
  // WHY CANT I DO THIS
  (*newBoard).board = malloc(h * sizeof(newBoard->board));
  for (int b = 0; b < h; b++) {
    (*newBoard).board[b] = malloc(w * sizeof(newBoard->board[b]));
  }
  // Step 2: Fill in all squares with UNKNOWN
  for (int i = 0; i < h; i++) {
    for (int j = 0; j < w; j++) {
      newBoard->board[i][j] = UNKNOWN;
    }
  }
  // Step 3: Call addRandommine function
  int k = 0;
  while (k < numMines) {
...
int countMines(board_t * b, int x, int y) {
  // check if any of the 8 boxes is a mine:
  int total = 0;
